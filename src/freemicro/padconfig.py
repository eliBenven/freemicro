...
def _parse_binding(input_id: str, raw: Any) -> Action:
    ...
    if kind == "hold" and (params.get("latch") or params.get("double_tap")):
        # Both the latch machine and double-tap detection need a real release to
        # time their windows, and these inputs report one event and never a
        # key-up - the same reason they cannot carry a "light". Refused at load
        # time, not left to fail silently the first time the key is pressed.
        members = parse_chord_id(input_id) if is_chord_id(input_id) else (input_id,)
        momentary = [m for m in members if m in _MOMENTARY_INPUTS]
        if momentary:
            what = "latch" if params.get("latch") else "double-tap"
            raise PadConfigError(
                f"binding {input_id!r} cannot {what}: {', '.join(momentary)} "
                "report one event and no release, so the window could never "
                f"resolve. {what.capitalize()} belongs on an input you can hold."
            )
    light = (
